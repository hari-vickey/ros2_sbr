...
class SelfBalancingBot(Node):
    ...
    def __init__(self):
        ...
        # PID Variables
        self.set_pos, self.limit = 0.0, 100
    ...
    def pos_callback(self, msg):
        ...
        # Limiting Error Sum
        if int(self.error_sum) not in range(-self.limit, self.limit):
            if self.error_sum <= 0:
                self.error_sum = self.error_sum + self.limit
            else:
                self.error_sum = self.error_sum - self.limit
        ...
        if abs(self.error) > 0.01:
            ...
            self.velocity.linear.x = p + i + d
        else:
            self.velocity.linear.x, p, i = 0.0, 0.0, 0.0
            self.prev_error, self.error, d = 0.0, 0.0, 0.0
